package com.nipuna777.sinhalawatch
...
class MyWatchFace : CanvasWatchFaceService() {
    ...
    inner class Engine : CanvasWatchFaceService.Engine() {
        ...
        private lateinit var mTextPaintSub: Paint
        ...
        override fun onDraw(canvas: Canvas, bounds: Rect) {
            ...
            val text = if (mAmbient)
                getHourString(mCalendar.get(Calendar.HOUR))
            else
                getHourString(mCalendar.get(Calendar.HOUR))

            canvas.drawText(text, mXOffset, mYOffset, mTextPaint)
            canvas.drawText(getMinuteString(mCalendar.get(Calendar.MINUTE)), mXOffset, mYOffset.plus(80), mTextPaintSub)
        }
